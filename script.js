...
function persistCurrentTextFromDoc(label = "地点协作内容已实时同步", options = {}) {
...
function persistCurrentDayTextFromDoc(label = "当天文本协作内容已实时同步", options = {}) {
  const { refreshViews = true, scheduleSave = true, updateStatus = true } = options;
...
async function refreshLiveCollabStateBeforeRemoteSave(label = "保存前已刷新协作快照") {
  ...
  if (collabTextDoc && collabTextStopId) {
    persistCurrentTextFromDoc(label, { refreshViews: false, scheduleSave: false, updateStatus: false });
  }
  if (collabDayTextDoc && collabDayTextDayId) {
    persistCurrentDayTextFromDoc(label, { refreshViews: false, scheduleSave: false, updateStatus: false });
  }
