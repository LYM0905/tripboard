...
function commentThreadCount(comments = []) {
  return commentRootsAndReplies(comments || []).roots.length;
}

function stopCommentThreadCount(stop = {}) {
  return commentThreadCount(stop.comments || []);
}

function dayCommentRemovalSummary(day = {}) {
  const dayThreads = commentThreadCount(day.comments || []);
  const stopThreads = (day.stops || []).reduce((sum, stop) => sum + stopCommentThreadCount(stop), 0);
  const blockThreads = normalizeDayBlocks(day.blocks || []).reduce((sum, block) => sum + dayBlockCommentThreadCount(block), 0);
  return { dayThreads, stopThreads, blockThreads, totalThreads: dayThreads + stopThreads + blockThreads };
}

function confirmDeleteCommentedStop(stop = {}, actionLabel = "删除") {
  const totalThreads = stopCommentThreadCount(stop);
  if (!totalThreads) return true;
  return window.confirm(`「${stop.title || "这个地点"}」里还有 ${totalThreads} 条评论线程。${actionLabel}后这些讨论也会被移除，确定继续吗？`);
}

function confirmDeleteCommentedDay(day = {}, actionLabel = "删除") {
  const summary = dayCommentRemovalSummary(day);
  if (!summary.totalThreads) return true;
  const parts = [
    summary.dayThreads ? `当天批注 ${summary.dayThreads} 条` : "",
    summary.stopThreads ? `地点评论 ${summary.stopThreads} 条` : "",
    summary.blockThreads ? `协作块批注 ${summary.blockThreads} 条` : "",
  ].filter(Boolean).join("、");
  return window.confirm(`「${day.title || day.label || "这一天"}」里还有 ${parts}。${actionLabel}后这些讨论也会被移除，确定继续吗？`);
}
...
dom.deleteDayBtn.addEventListener("click", async () => {
  ...
  if (!confirmDeleteCommentedDay(deletedDay, "删除")) {
    dom.saveState.textContent = "已取消删除含讨论的日期";
    return;
  }
  if (!mutate(label, () => {
